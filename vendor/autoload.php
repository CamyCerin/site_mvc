...
class Autoload
{
    ...
    public static function className($className)
    {
        ...
        $tab = explode('\\', $className); // explode permet de prendre la chaine (string) et de la transformer en tableau ARRAY. on cherche le caractère \ mais si on en met qu'un seul c'est comme si on voulais échaper la quote (') en php, alors dans ce cas précis il faut mettre 2 anti slash \\.
		// print '<pre>'; print_r($tab); print '</pre>';
		// if(in_array(end($tab), get_declared_classes())){ echo '<h1>' . end($tab) . '</h1>'; $path = " "; }
		
        if($tab[0] == 'Backoffice') // l'explode nous permet de savoir si l'on doit reculer d'un dossier pour aller cherche un bundle (c'est à dire un module spécifique)
