...
class Controller {
    ...
    public function getRepository($table) // on récup  --- méthode qui permet d'instancier un objet
    {
        $class = 'Repository\\' . $table . 'Repository';
        if(!isset($this->$table))
		{
            $this->table = new $class; // on instancie un objet  (s'il n'existe pas) - donc la 1ere fois, oui!
        }
    ...
    public function render($layout, $template, $parameters = array()) // permet de rendre un affichage
    {
        ...
        $dirFile = str_replace('Controller', '', $ex[2]);  // on retire le mot Controller grâce à str_replace
		// echo $dir_file;
