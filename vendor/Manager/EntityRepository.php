...
//-------
    ...
    {
        $q = $this->getDb()->query('SELECT * FROM ' . $this->getTableName() . ' WHERE id' . '= ' . (int) $id); // id le premier champ est toujours le nom de la table. Caster en int permet d'éviter des erreurs de requete sql.
        $q->setFetchMode(PDO::FETCH_CLASS, 'Entity\\' . $this->getTableName());  // PDO::FETCH_CLASS permet d'instancier un objet, setFetchMode permet de prendre les resultats de la requête et d'affecter les propri�t�s de l'objet (il faut pour cela que l'orthographe des noms des colonnes/champs SQL correspondent aux propri�t� de l'objet)
        $r = $q->fetch(); 
...
//-------
	public function register()
	{
        ...
        if(isset($_POST['title']) || isset($_POST['content'])){
            $_POST['author_id'] = $_SESSION['user']->getId();
            $_POST['created_date'] = $date->format('Y-m-d H:i:s');
        }

		$q = $this->getDb()->query('INSERT INTO '. $this->getTableName() . '(' . implode(',',array_keys($_POST)) . ') VALUES (' . "'" . implode("','", $_POST) . "'" . ')'); // array_keys me permet de parcourir les indices plutôt que les valeurs pour annoncer les champs.
